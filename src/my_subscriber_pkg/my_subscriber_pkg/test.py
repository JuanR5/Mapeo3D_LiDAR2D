...
class GoToGoal(Node):
    ...
    def move_angle(self):
        ...
        # Check for obstacles and calculate theta if any obstacle is detected
        obstacle_detected = False
        theta = 0
        for i, range_val in enumerate(self.scan_ranges):
            if 0 < range_val < 0.5:
                obstacle_detected = True
                self.xi += range_val * np.cos((2*np.pi*i)/490)
                self.yi += range_val * np.sin((2*np.pi*i)/490)
                theta = np.arctan2(self.yi/self.xi) - np.deg2rad(135)
                break
